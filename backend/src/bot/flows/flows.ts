...
const invokeGotoFlow = (fn: unknown, ...params: unknown[]) => {
	...
	async (ctx, { gotoFlow, state }) => {
		...
		switch (user.flujo) {
			case "assistantFlow":
				...
				return invokeGotoFlow(gotoFlow, assistantFlow);
			case "testFlow":
				console.log("testFlow");
				return invokeGotoFlow(gotoFlow, testFlow);
			case "agendFlow":
				console.log("agendFlow");
				return invokeGotoFlow(gotoFlow, agendFlow);

			case "finalFlow":
				console.log("finalFlow");
				return invokeGotoFlow(gotoFlow, finalFlow);

			default:
				console.log("registerFlow");
				return invokeGotoFlow(gotoFlow, registerFlow);
		}
	}
);
...
export const assistantFlow = addKeyword(utils.setEvent("ASSISTANT_FLOW")).addAction(
	async (ctx, { flowDynamic, gotoFlow, state }) => {
		...
		if (user.ayudaPsicologica == 2) {
			...
			const assist = await apiAssistant2(ctx.from, ctx.body);
...
export const agendFlow = addKeyword(utils.setEvent("AGEND_FLOW")).addAction(
	async (ctx, { flowDynamic, state }) => {
		const user = state.get("user");
		const msgAgend = await apiAgend(ctx.from, ctx.body);
