...
async function register(conversationHistory: ConversationMessage[], number: string) {
	...
	const responseJson = safeJsonParse<{
		...
		documento: string
	}>(jsonRegister.choices[0].message.content ?? '')
	if (!responseJson) {
		throw new Error('No se pudo registrar al usuario, el modelo no devolvió datos válidos')
	}
