...
export async function apiHorarios(msg: string): Promise<{ fechaHora?: Date } & Horario> {
	try {
		const hist: Array<{ role: 'system' | 'user'; content: string }> = [
			{
				role: 'system',
		...
		return JSON.parse(content) as Horario & { fechaHora?: Date }
	} catch (error) {
